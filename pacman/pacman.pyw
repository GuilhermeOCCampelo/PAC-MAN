...
class QuizSystem:
    ...
    def draw(self, screen, screen_size):
        ...
        instruction = self.font_small.render("Pressione A, B, C ou D", True, (255, 255, 0))
...
class game ():
    ...
    def MoveScreen (self, newX, newY ):
        ...
        self.screenNearestTilePos = (int(newY / 16), int(newX / 16))
...
def CheckInputs(): 
    ...
    # SE QUIZ ATIVO, PROCESSA RESPOSTA
    if quiz_system and quiz_system.active and not quiz_system.answered:
        keys = pygame.key.get_pressed()
        
        if keys[pygame.K_a]:
            if quiz_system.check_answer('A'):
...
# --- INÍCIO DA MODIFICAÇÃO PARA TELA MAIOR ---

# Tamanho lógico do jogo (original 336x368)
logical_screen_size = thisGame.screenSize 

# Fator de escala (mude 2 para 3, etc. para maior)
SCALE_FACTOR = 2 
scaled_window_size = (logical_screen_size[0] * SCALE_FACTOR, logical_screen_size[1] * SCALE_FACTOR)

# 'window' é a janela real que o usuário vê (tamanho escalonado)
window = pygame.display.set_mode( scaled_window_size, pygame.DOUBLEBUF | pygame.HWSURFACE )

# 'screen' é a superfície INTERNA onde o jogo desenha (tamanho original)
# (Isso sobrescreve a variável 'screen' global definida no início)

# A LINHA "global screen" FOI REMOVIDA DAQUI
screen = pygame.Surface(logical_screen_size)
...
while True: 
    ...
    # DESENHAR QUIZ SE ATIVO
  # CÓDIGO MODIFICADO (substitua o bloco acima)
    if quiz_system and quiz_system.active:
      quiz_system.draw(screen, thisGame.screenSize)
  
  # --- INÍCIO DA MODIFICAÇÃO PARA ESCALAR ---

  # Desenha a 'screen' (lógica) na 'window' (real), escalonando-a
    pygame.transform.scale(screen, scaled_window_size, window)
    pygame.display.flip()

    # O código original era apenas:
    # pygame.display.flip()
